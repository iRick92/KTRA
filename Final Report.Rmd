...
author: Hoon Kim, Ricky Cai, Ricky Yavits, Sherry Fan
geometry: margin=2cm
output:
  pdf_document:
    pandoc_args: [
...
The linear assumption is the requirement that the continuous variable need to be in a linear correlation with the observed times of events. Here all 3 models passed this assumption but there was some non linearity showing.  
...
![AMR model using total ElliPro class 2 score with median C index around 0.61 and 0.64 respectively](screenshots/AMR_Boxplot.png)
...
# Idea - When summing up the mismatches, count for that patient: Number of High misamtches (Number 4): 8, NUmber of Intermediate mismatches (Number 4): 2.. 
# To make it clear which why it is predicted the transplant will be good or bad for transparency on how the predictions are made
...
colnames(eplets_data)[87] <- 'Total_EliPro_Score_class1'
colnames(eplets_data)[88] <- 'Total_EliPro_Score_class2'
